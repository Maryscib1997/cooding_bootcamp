...
const PATCH = async (endpoint, data) => {
  ...
  console.log("Dati aggiornati parzialmente:", data);
  console.log("Risposta della API:", responseData);
...
const PUT = async (endpoint, data) => {
  ...
  console.log("Dati aggiornati completamente:", data);
  console.log("Risposta della API:", responseData);
...
// Esempio di utilizzo delle funzioni PATCH, PUT e DELETE
...
POST('/movie', exampleData).then(id => {
  ...
  // Esempio di PATCH
  PATCH(`/movie/${id}`, updateDataPartial).then(updatedData => {
    console.log("Dati parzialmente aggiornati:", updatedData);
    
    // Verifica l'aggiornamento parziale
    GET(`/movie/${id}`).then(data => {
      console.log("Dati del film aggiornati (parziale):", data);
    });

    // Esempio di PUT
    PUT(`/movie/${id}`, updateDataFull).then(updatedData => {
      console.log("Dati completamente aggiornati:", updatedData);

      // Verifica l'aggiornamento completo
      GET(`/movie/${id}`).then(data => {
        console.log("Dati del film aggiornati (completo):", data);

        // Esempio di DELETE
        DELETE(`/movie/${id}`).then(() => {
          // Verifica la cancellazione
          GET(`/movie/${id}`).then(data => {
            if (!data) {
              console.log("Il film è stato cancellato con successo.");
            } else {
              console.log("Dati del film dopo la cancellazione:", data);
            }
          }).catch(error => {
            console.error("Errore durante la verifica della cancellazione:", error);
          });
        });
      });
    });
